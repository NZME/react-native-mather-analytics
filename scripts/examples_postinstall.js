#!/usr/bin/env node
  ...
  (() => {
    ...
    try {
      removeFileDirectoryRecursively(path.resolve(exampleDir, 'node_modules', require('../package.json').name, 'example'));
    } catch (e) {}
    try {
      removeFileDirectoryRecursively(path.resolve(exampleDir, 'node_modules', require('../package.json').name, '.idea'));
    } catch (e) {}
  })();
